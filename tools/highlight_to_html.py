...
The `qn` binary is found via $QN_BIN, then PATH, then a sibling quoin
...
def find_qn(explicit=None):
    ...
    roots = [
        os.path.normpath(os.path.join(here, "..", "..", "quoin")),
        os.path.expanduser("~/code/quoin"),
    ]
    for root in roots:
...
def css_block():
    lines = ["  /* Quoin standard syntax palette — generated by tools/highlight_to_html.py --css */",
             "  /* source of truth: quoin/crates/quoin-syntax/src/highlight.rs colors_for() */"]
