 ...
 * @returns {string}
 */
export function highlight_stylesheet() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.highlight_stylesheet();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
 ...
 * `Runtime.exit:`.
 * @param {string} source
 * @param {number | null | undefined} max_batches
 * @param {Function} on_output
 * @returns {string}
 */
export function run(source, max_batches, on_output) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(source, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.run(ptr0, len0, isLikeNone(max_batches) ? Number.MAX_SAFE_INTEGER : (max_batches) >>> 0, on_output);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('quoin_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
